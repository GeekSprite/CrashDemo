...
//    else
//    {
//        NSLog(@"Crash matches filter and it will not be processed.");
//    }
...
- (NSDictionary *)binaryImagesForStackTrace:(NSArray *)stackTrace
{
    ...
    for (uint32_t i = 0; i < imageCount; i++)
    {
        ...
        {
            NSLog(@"Image Name can not be retrieved!");
            continue;
        }
        ...
        NSLog(@"Image Name is in the stack trace, so it will be used!\n%@", imageName);

        const struct mach_header* imageHeader = _dyld_get_image_header(i);
        if (imageHeader == NULL)
        {
            NSLog(@"Image Header can not be retrieved!");
            continue;
        }
        ...
        if (!imageUUID)
        {
            NSLog(@"Image UUID can not be retrieved!");
            continue;
        }
